...
#define HW_USE_HC06  0
#define HW_USE_HM10_HMSOFT 1
...
const char PKT_CMD_ACLEAR  = 'C'; // number (ms before auto clear, 0=disable)
...
struct globalData 
{
  ...
  uint16_t autoClear;   // msg auto clear in ms
...
uint32_t messageTimeStart = millis();
// Application Code ------------------
...
void loop()
{
  ...
  if(G.autoClear > 0 && G.msg != ""){
    if(messageTimeStart + G.autoClear < millis()){
      strcpy(newMessage, "");
      newMessageAvailable = true;
    }
  }
